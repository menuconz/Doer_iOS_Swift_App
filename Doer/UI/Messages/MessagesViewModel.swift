...
struct ReplyMentionState {
    var suggestions: [MentionSuggestion] = []
    var showSuggestions: Bool = false
    var mentionedContractorEmail: String = ""
}

struct SelectedFileItem: Identifiable {
    let id = UUID()
    let url: URL
    let fileName: String
    let filePath: String
...
class MessagesViewModel {
    ...
    func loadThreads() {
        Task { @MainActor in
            let result = await emailMessageRepository.getAllEmailMessageByShiftId(shiftId: shiftId)
            switch result {
    ...
    func sendReply(_ threadRootEmailId: Int) {
        ...
        Task { @MainActor in
            ...
            let result = await emailMessageRepository.sendEmailReply(request: request)
            switch result {
            case .success:
    ...
    func sendNewMessage() {
        ...
        Task { @MainActor in
            ...
            if !selectedFiles.isEmpty {
                ...
                result = await emailMessageRepository.sendNewEmailWithAttachments(
                    jobId: jobIdToUse,
                    toEmail: toEmail,
                    subject: subject,
                    body: text,
